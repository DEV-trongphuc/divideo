...
def pick_runtime_dtype(device: str, configured_dtype: str) -> str:
    ...
    Users can override with ``VOXCPM_MPS_DTYPE`` (e.g. ``bfloat16``) or
    ``VOXCPM_CPU_DTYPE`` (e.g. ``float32``) when they want to test or coerce
    specific dtypes.
    """
    if device == "cpu":
        override = os.environ.get("VOXCPM_CPU_DTYPE", "").strip().lower()
        if override:
            if override not in _VALID_DTYPE_OVERRIDES:
                raise ValueError(
                    f"VOXCPM_CPU_DTYPE='{override}' is not one of "
